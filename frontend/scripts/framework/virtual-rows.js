/* Purpose: Windowed <tbody> renderer for the rp-redtable — mounts only the rows near the viewport (+overscan), spacer <tr>s carry off-window height; factory createVirtualRows (no registry, no CSS family of its own).
   Doc: docs/internal/code/frontend/scripts/framework/virtual-rows.md */
// ─────────────────────── virtual-rows (framework) ───────────────────────
//
// Windowed <tbody> renderer for the rp-redtable. Mounts only the rows
...
//
// Assumptions:
//   • Uniform row height (rp-redtable cells are `white-space: nowrap`,
//     one line each — the rp-table base in styles/framework/table.css).
//     Pass the height for the active density; call `remeasure()` after a
//     density change.
//   • A single scroll container (the rp-table-wrap, or whatever ancestor
//     the caller resolves) wraps the <table>; passed in as `scroller`.
//
...
//
// Spacer class: the spacer <tr> carries the class `rp-redtable-spacer`,
// which has NO CSS rule — it is styled ENTIRELY inline below (the
// `content-visibility:visible` opt-out + the <td> height/padding/border).
// The caller's first-real-row measurement query is therefore
// `tr:not(.rp-redtable-spacer)`.
//
// API:
