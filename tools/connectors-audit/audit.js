 ...
 * Doc: self-documenting (header below). Backend connectors live in
 *      backend/crates/api/src/ (see docs/internal/code/backend/). */
/* ──────────────────────────────────────────────────────────────────────────
   ...
   the next connector (postgres-pull, mysql-pull, …) can't quietly reintroduce
   the bypass.
   ...
   A .rs file whose path is under a `connectors/` segment OR whose basename
   contains `loader` / `connector` (mysql_loader.rs + connectors_core.rs today;
   *_loader.rs / *_connector.rs and any future connectors/ packages tomorrow).
   Deliberately broad + name-based so a new connector is caught the day it lands,
   ...
   `db::find_*`) are fine.
     ...
     yellow — connector does neither (pure consumer / transport-only, an
              export/reverse loader that writes bytes OUT, the SSRF/TLS
              admission gate, or a connector that doesn't ingest yet) → read it.
   ...
   ── outputs ──────────────────────────────────────────────────────────────
   stdout summary + audit.html. Exit 1 if any red (fails `sh tools/ci.sh`).

   Usage:  node tools/connectors-audit/audit.js
   ────────────────────────────────────────────────────────────────────────── */
'use strict';
...
/* Lean tree has a single crate root; a `connectors/` package dir may land later
   (walkRust guards every path with existsSync, so listing one that's absent is
   harmless and future-proofs the scan). */
var SCAN_DIRS = [
...
function isConnector(relPath) {
  ...
  // any future connectors/ packages). Note: connectors_core.rs (the shared
  // SSRF/TLS admission gate) is NOT `connectors.rs`, so it IS scanned — and it
  // correctly reads yellow (no mutation, no pipeline:: — a primitive to review).
