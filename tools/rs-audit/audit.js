 ...
 * Doc: docs/internal/code/backend/ (audit-suite docs land here as the suite matures). */
/* ──────────────────────────────────────────────────────────────────────────
   RedPash Rust — refactoring audit  (LEAN tree, lean-adapted)
   ---------------------------------------------------------------------------
   A static scan of backend/. Codifies the manual Rust audit into a repeatable
   tool — re-run it as a refactor lands to watch the mass come down.

   LEAN NOTE: scanning logic is path-agnostic. It walks backend/ for *.rs and
   derives the crate from `crates/<name>/…` — so it works unchanged on the lean
   layout backend/crates/{api,data,shared}/src/ (+ the api/src/files/ and
   data/src/{parse,steps}/ sub-dirs). The PATTERNS / ACK catalog below carries
   forward from prerelease; entries whose helpers/tables no longer exist in the
   lean cut simply report 0 (harmless — a live-0 counter, not a false alarm).
   Notes that referenced prerelease-only paths (e.g. the old routes/ module)
   have been re-pointed at the lean equivalents.

   Three views + Patterns:
     1. Files          — every .rs file: crate, LOC. Flags hotspots (LOC>600).
     2. Repeated lines — a substantial source line recurring 4+ times. The
                         copy-paste surface. Caveat: framework boilerplate
                         (Axum extractor signatures) recurs by design and
                         shows here too — a candidate list, not a verdict.
     3. Big matches    — `match` blocks over 60 lines (written as code, not
                         a table).
     4. Patterns       — named antipatterns / helper-usage / ACK scans.
...
/* ── pattern catalog ─────────────────────────────────────────────────────────
   Named antipatterns + helper-usage markers carried forward from the
   prerelease dedup audit. Each entry is counted across the scan; the report's
   "Patterns" tab shows current state.
   ...
   Statuses:
     ...
     live      — observability for already-extracted helpers (count = healthy
                 reuse, not antipattern). On the lean cut some of these helpers
                 may not have been ported yet → a 0 count is expected, not a
                 regression.
   ────────────────────────────────────────────────────────────────────────── */
var PATTERNS = [
  ...
  { name: 'sqlx error .map_err chain', status: 'extracted',
    ...
    saving: 1, notes: 'prerelease commit 194ee26 retired 134 sites' },
  { name: 'bare COUNT(*) inline scalar', status: 'extracted',
    ...
    saving: 3, notes: 'prerelease commit 21dcfc3 retired 14 sites' },
  { name: 'duplicate paginate() / build_page<T>() definitions', status: 'extracted',
    ...
    saving: 12, notes: 'only 1 def per name expected (in pagination.rs) — 0 if not yet ported to lean' },
  ...
  { name: 'db::count_total usage', status: 'live',
    ...
    saving: 0, notes: 'count tracks reuse spread (0 = helper not yet in lean)' },
  ...
  /* hydrate is the canonical "give me the parsed Polars frame" entry. On the
     lean cut it lives in the api crate's files module (api/src/files/, which
     replaces prerelease's routes/files.rs). The live counter tracks reach; the
     per-file breakdown surfaces cross-module callers — those flip from healthy
     reuse to layer-violation when the canonical Reader/DataSource read path
     lands. Until then, callers outside the files module warrant a look. */
  { name: 'hydrate(state, …) usage', status: 'live',
    rx: /\bhydrate\s*\(/g,
    helper: '(canonical data-frame entry — lives in the api crate files module)',
    saving: 0, notes: 'cross-module callers (outside api/src/files/) are the interesting signal' },
...
var ACK_SCANS = [
  /* Every `FROM project_files` (case-insensitive) must declare its
     ...
     type intent risks the spec-only-rows-leaking-into-data-paths class.
     Three valid intents:
     ...
     (project_files is still the polymorphic table on the lean cut.) */
  { name: 'project_files query intent (PROJECT-FILES-ACK)',
    ...
    notes: 'forces type-shape intent at every project_files query site' },
];
