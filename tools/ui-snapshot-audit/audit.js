 ...
 * Doc: tools/ui-snapshot-audit/README.md (this dir) — see the header block below. */
/* ──────────────────────────────────────────────────────────────────────────
   RedPash UI-snapshot audit  (LEAN tree)
   ---------------------------------------------------------------------------
   Reads UI snapshot JSONs captured by the frontend's `?audit=1` mode
   (the `captureSnapshot()` self-report — Layer 2a), normalises them into a
   ...
   The tool itself is namespace- and route-agnostic: route / theme / state /
   atom-selectors all come from the captured JSON body, never hardcoded here.
   It works unchanged whether the catalog is `rp-*` (lean) or any other.
