...
| `case_create` | File a new RedPash case. Returns the created Case row including its `CAS_<rid>`. `title` required; type / priority default backend-side. |
...
The case tools call the canonical `/api/cases` routes on a running
RedPash backend. Two env vars configure the bridge; both have safe
defaults for the canonical-host case.

| env | default | purpose |
|---|---|---|
| `REDPASH_API_BASE` | `http://localhost:8080/api` | API base (with the `/api` suffix). |
| `REDPASH_API_SESSION` | — | `rp_session` cookie value. Required: the cases routes are auth-gated. Generated by the canonical host (`POST /api/auth/dev-login` returns it on the `Set-Cookie` header) and pinned in the process env. |

When `REDPASH_API_SESSION` is unset, case tools will fail with HTTP
401. That's by design — never silently fall back to a different
identity than the configured agent. Errors come back as
`isError: true` text content carrying the HTTP status + the response
body, so the caller sees what went wrong.

In **stdio mode**, the MCP server runs as a subprocess of the agent's
Claude Code instance, so the env is scoped per-agent — your session
cookie identifies your agent on every call. In **HTTP-transport
mode** (`REDPASH_MCP_PORT` set), the canonical server's env identifies
the canonical user; cross-agent identity per call is a v2 design.
