...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   Usage:  node audit-bro.js [partialsDir]
   Output: ./audit-bro.html

   Fork of audit.js with one fix — see the FIX comment by the emit block at
   the bottom of this file: the original called renderHtml(data) BEFORE the
   `var CSS` / `var JS` assignments ran, so the report shipped
   `<style>undefined</style>` + `<script>undefined</script>` — an unstyled,
   non-interactive page. The emit is moved to the end so CSS + JS exist.
   ────────────────────────────────────────────────────────────────────────── */
'use strict';
