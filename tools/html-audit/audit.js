 ...
 * Doc: docs/internal/code/frontend/README.md (framework-first layer) */
/* ──────────────────────────────────────────────────────────────────────────
   RedPash HTML component-extraction audit
   ---------------------------------------------------------------------------
   Parses every page/component HTML file under the lean frontend tree, hashes
   every element subtree three ways, and surfaces reusable-component candidates
   ranked by how many lines extracting them would save.
   ...
   LEAN ADAPTATION: prerelease scanned a single flat `frontend/partials/` tree
   (since retired). The lean cut splits HTML across frontend/framework/<component>
   and frontend/apps/<app>/<page>/<page>.html (the built pages registered in
   frontend/framework/boot/apps.js). Both roots are scanned recursively; member
   paths are reported relative to frontend/ so a `framework/…` vs
   `apps/<app>/<page>/…` origin stays legible in every candidate. The shell
   files frontend/index.html + frontend/framework-sandbox.html are NOT partials
   (they are the bootstrap shell / live sandbox) and are excluded. CSS namespace
   is rp-* (rt-* retired), so compName strips rp-/sp- only. Pass a directory
   argument to scan a single subtree instead.

   Usage:  node tools/html-audit/audit.js [scanDir]
   Output: ./audit.html + ./audit.json  (next to this script)
           audit.json is the source-of-truth payload an audit-ingest step
           reads to persist audit.run + audit.finding rows (same shape as the
           sibling lean audits: rs-audit / css-audit / doc-coverage-audit).
...
/* Lean FE layout: HTML lives under frontend/framework/<component>/ and
   frontend/apps/<app>/<page>/ (the prerelease frontend/partials tree is
   retired). Both roots are scanned; member paths are reported relative to
   frontend/ so the framework-vs-app origin is visible in every candidate.
   A directory arg overrides the default roots (and roots reporting). */
var FE_ROOT = path.join(__dirname, '..', '..', 'frontend');
var SCAN_DIRS;
var REL_BASE;
if (process.argv[2]) {
  SCAN_DIRS = [path.resolve(process.argv[2])];
  REL_BASE = SCAN_DIRS[0];
} else {
  SCAN_DIRS = [path.join(FE_ROOT, 'framework'), path.join(FE_ROOT, 'apps')]
    .filter(function (d) { return fs.existsSync(d); });
  REL_BASE = FE_ROOT; // report "framework/…" / "apps/<app>/<page>/…"
}
/* Shell files that are NOT extraction partials (bootstrap shell + live
   component sandbox); skip them when scanning the whole frontend tree. */
var SHELL_SKIP = { 'index.html': 1, 'framework-sandbox.html': 1 };
...
function walk(dir, acc) {
  fs.readdirSync(dir, { withFileTypes: true }).forEach(function (e) {
    ...
    else if (e.isFile() && /\.html?$/i.test(e.name) && !SHELL_SKIP[e.name]) acc.push(full);
  });
  return acc;
}
...
/* ── run: parse every page/component HTML ────────────────────────────────── */
if (!SCAN_DIRS.length) {
  console.error('No scan roots found under ' + FE_ROOT + ' (expected framework/ or apps/).');
  process.exit(1);
}
console.log('Scanning ' + SCAN_DIRS.map(function (d) { return path.relative(process.cwd(), d); }).join(', ') + ' …');
var files = SCAN_DIRS.reduce(function (acc, d) { return walk(d, acc); }, []).sort();
if (!files.length) { console.error('No .html files found.'); process.exit(1); }

var roots = [];
var fileMeta = [];
files.forEach(function (full) {
  // Report relative to frontend/ → "apps/studio/workspace/workspace.html",
  // "framework/modal/modal.html"; relative to the arg dir when one is given.
  var rel = path.relative(REL_BASE, full).split(path.sep).join('/');
...
function compName(root) {
  // lean namespace is rp-*; sp-* kept for any sandbox-prefixed atom. rt-* is
  // retired so it never appears.
...
var data = {
  ...
  // legible across the lean tree: the roots we scanned (relative to frontend/).
  scanDirs: SCAN_DIRS.map(function (d) { return path.relative(FE_ROOT, d).split(path.sep).join('/') || '.'; }),
  partialsDir: REL_BASE, // kept for ingest back-compat (single base path)
  stats: {
...
/* `data` is the source-of-truth payload an audit-ingest step reads to persist
   audit.run + audit.finding rows (same shape as sibling lean audits). */
