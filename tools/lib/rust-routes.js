/* Purpose: shared Rust axum route extractor — dir-modules, depth-N nests, HTTP methods, generic nest-gates.
   Doc: docs/internal/code/tools/lib/rust-routes.md */
/* ──────────────────────────────────────────────────────────────────────────
   RedPash shared route extractor — the single code-side API surface parser
   ---------------------------------------------------------------------------
   The js-rust-boundary contract crosses at /api routes + shared DTOs. Several
   tools need the *code-side* route set: crossing-audit (JS↔Rust seam),
   list-endpoint-rbac-audit (RBAC posture), api-doc-audit (doc↔code parity).
   This lib is the one parser they all consume, so the extraction logic lives
   in exactly one place (and a fix lands for every consumer at once).

   What it does that the old per-tool extractors didn't:
     • Resolves DIRECTORY modules — `files` → routes/files/mod.rs (not the
       non-existent routes/files.rs that crossing-audit silently skipped).
     • Recurses `.nest()` to arbitrary depth — the generic `/:rid/members`
       CRUD nested under projects/companies/cases/teams expands under each
       parent (four distinct route sets), which a top-level-only nest regex
       misses entirely.
     • Captures the HTTP method(s) per `.route("/p", get(h).post(h2))` — the
       method dimension the seam audit never had.
     • Detects the platform-admin gate GENERICALLY on any `.nest()` whose
       paren-matched span carries a GATE_MW `.layer(...)` (today /metrics,
       /monitoring, /admin — not hardcoded), so a newly-gated nest is picked
       up automatically. Absorbs list-endpoint-rbac-audit::parseNestGates.

   Returns one record per (method, path):
     { method, path (raw, e.g. /api/companies/:rid/members/:member_id),
...
/* Middleware names that, when chained as a `.layer(...)` onto a nest, gate the
   whole subtree. Keep in sync with backend routes/mod.rs. */
var GATE_MW = ['require_platform_admin_mw'];

var ROUTES_REL = 'backend/crates/api/src/routes';
...
function moduleFile(routesDir, mod) {
  ...
  if (fs.existsSync(flat)) return { abs: flat, rel: 'routes/' + mod + '.rs' };
  var dirMod = path.join(routesDir, mod, 'mod.rs');
  if (fs.existsSync(dirMod)) return { abs: dirMod, rel: 'routes/' + mod + '/mod.rs' };
  return null;
}
...
/* the module search text backing a route file (routes/users.rs → users module;
   routes/files/mod.rs → the whole files/ dir). Local request/response structs
   are defined here — resolved BEFORE the shared crate so a name shared across
   modules (e.g. two different `PatchUserBody`s) picks the handler's own one. */
...
function rustRoutes(root) {
  ...
  var modRaw = read(path.join(routesDir, 'mod.rs'));
  if (modRaw == null) throw new Error('routes/mod.rs not found under ' + routesDir);
  var modStripped = strip(modRaw);

  var out = [];
  var visited = new Set();
  parseTopNests(modStripped, modRaw).forEach(function (n) {
    var rbac = n.gated
      ? { source: 'nest-layer', hint: 'platform_admin', gated: true }
      : { source: 'handler', hint: null, gated: false };
    extractModule(routesDir, n.module, joinPath('/api', n.prefix), rbac, out, visited, 0);
  });
