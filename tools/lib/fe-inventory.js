...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   whatever the author forgot (a modal, the omnisearch dropdown, …). Completeness
   has to be MECHANICAL: enumerate every component that exists IN THE CODE, so
   nothing can be silently dropped. This lib is the single source of that
   enumeration; the coverage audit (tools/ui-doc-audit) consumes it — same shape as
   how tools/lib/rust-routes.js serves the route lane.

   LEAN ADAPTATION (2026-06-16): the predecessor scanned a single flat
   frontend/styles + frontend/partials + frontend/scripts tree and used the
   rt-/rp-/ds-/ws- prefix soup. The lean cut CO-LOCATES the front-end:
     • frontend/framework/<name>/<name>.{js,css}[,.html] — one self-registering
       component per dir (`register("<name>", mount…)`), sole owner of `.rp-<name>-*`.
     • frontend/apps/<app>/<page>/<page>.{js,css,html}   — page surfaces (pg-/rp-).
     • frontend/styles/{tokens,prefs,base,main}.css       — foundation + @import manifest.
   There are no partials/ and no scripts/ dirs. The namespace is rp- (components)
   + pg- (page-scoped); rt-/ds-/ws- are RETIRED (0 occurrences) so they are dropped
   from the token regex. vendor/ wasm/ wasm-src/ tests/ dist/ node_modules/ are
   skipped (matches css-audit / ui-fork-audit SKIP_DIRS — same authored surface).

   THREE SOURCES (union → truly complete):
     1. CSS class-families  (frontend .css)  — the strongest signal; anchors components
     2. HTML usage          (frontend .html)
     3. JS-rendered widgets (frontend .js, recursive) — the BLIND SPOT of a
        static scan (modals, pickers are injected by JS, not markup)
   ...
   (climbs through defined ancestors) but a block whose parent token is not itself a
   class stays its own block. Distinct blocks stay distinct; elements fold into their
   real block; `--modifier` is stripped by the token regex while `__element` is kept and
   folds to its BEM block. The dedup detector (ui-doc-audit) flags parallel blocks —
   grouping keeps them separate so the parallelism stays visible.
...
/* The authored FE roots in the lean tree. framework/ + apps/ hold the co-located
   js+css+html; styles/ holds the foundation sheets (tokens/prefs/base/main). */
var FE_ROOTS = ['framework', 'apps', 'styles'].map(function (d) { return path.join(FE, d); });
/* Not part of the authored UI surface — third-party glyphs, wasm payloads,
   build output, test fixtures, deps. Mirrors css-audit / ui-fork-audit. */
var SKIP_DIRS = { node_modules: 1, vendor: 1, wasm: 1, 'wasm-src': 1, tests: 1, dist: 1 };

/* Class tokens use rp-/pg- prefixes (rt-/ds-/ws- retired in the lean cut).
   Separator is a single `-` OR a BEM `__` element join (`_+`), so `rp-page__title`
   is captured WHOLE — the element is tracked + enters the `-title`/divergence
   analysis. A `--modifier` still breaks the match (`rp-toolbar--data` → `rp-toolbar`):
   after the first `-` the next char is another `-`, not [a-z0-9], so the run stops —
   modifiers stripped, elements kept. */
var CLASS_TOK = /\b(?:rp|pg)-[a-z0-9]+(?:(?:-|_+)[a-z0-9]+)*/g;
var CSS_DEF   = /\.((?:rp|pg)-[a-z0-9]+(?:(?:-|_+)[a-z0-9]+)*)/g;   // a class as a selector subject
...
function walk(dir, exts, out) {
  ...
  ents.forEach(function (e) {
    ...
    if (e.isDirectory()) {
      if (SKIP_DIRS[e.name] || e.name.charAt(0) === '.') return;
      walk(p, exts, out);
...
/* Collect files of the given exts across every authored FE root. */
function collect(exts) {
  var out = [];
  FE_ROOTS.forEach(function (r) { walk(r, exts, out); });
  return out;
}
...
function scan() {
  var cssFiles = collect(['.css']);
  var htmlFiles = collect(['.html']).concat(
    fs.existsSync(path.join(FE, 'index.html')) ? [path.join(FE, 'index.html')] : []);
  var jsFiles = collect(['.js']);
...
function inventory() {
  ...
  return {
    generatedFrom: '3-source scan (css+html+js) over frontend/{framework,apps,styles}',
...
/* ── dedup analysis (consumed by ui-doc-audit) ────────────────────────────────
...
function divergences(selIndex) {
  var out = [];
  selIndex = selIndex || cssSelectorIndex();
  Object.keys(selIndex).sort().forEach(function (cls) {
    var ctxs = {};
