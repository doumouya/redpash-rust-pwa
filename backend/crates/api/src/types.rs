...
pub async fn payload(pool: &PgPool) -> Result<Value, AppError> {
    ...
    let types: Vec<Value> = defs
        .into_iter()
        .map(|(type_id, display_name, display_name_plural, rid_prefix, grid_served, is_builtin)| {
            let fs: Vec<Value> = fields
                ...
                .map(|(_, key, label, ordinal, data_type, perm_class, field_group, scope, options)| {
                    ...
                    let label =
                        if label.is_empty() { title_case(key) } else { label.clone() };
                    let mut f = json!({
                        "key": key,
                        "label": label,
                        "data_type": data_type,
