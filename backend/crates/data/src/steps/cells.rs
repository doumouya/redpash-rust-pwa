...
pub(super) fn set_cell(df: DataFrame, params: &serde_json::Value) -> Result<DataFrame> {
    let row = params
        .get("row")
        .and_then(|v| v.as_u64())
        .ok_or_else(|| DataError::InvalidSpec("set_cell needs params.row: int".into()))?
        as i64;
    let column = params
        .get("column")
        .and_then(|v| v.as_str())
        .ok_or_else(|| DataError::InvalidSpec("set_cell needs params.column: string".into()))?;
    let height = df.height() as i64;
    if row < 0 || row >= height {
        return Err(DataError::InvalidSpec(format!(
            "set_cell row out of range: {row} (df height {height})"
        )));
    }
...
pub(super) fn cast(df: DataFrame, params: &serde_json::Value) -> Result<DataFrame> {
    ...
    df.lazy()
        .with_columns([expr.alias(column)])
        .collect()
        .map_err(DataError::from)
}
...
pub(super) fn change_case(df: DataFrame, params: &serde_json::Value) -> Result<DataFrame> {
    ...
    for c in df.get_columns() {
        ...
        let transformed = match mode {
            ...
            other => {
                return Err(DataError::InvalidSpec(format!(
                    "unsupported change_case mode: {other}"
                )))
            }
        };
...
pub(super) fn replace_text(df: DataFrame, params: &serde_json::Value) -> Result<DataFrame> {
    ...
    let expr = if is_regex {
        base.str()
            .replace_all(lit(find.to_string()), lit(replace.to_string()), false)
    } else {
        base.str()
            .replace_all(lit(find.to_string()), lit(replace.to_string()), true)
    }
    .alias(column);
    df.lazy()
        .with_columns([expr])
...
pub(super) fn fix_invalid(df: DataFrame, params: &serde_json::Value) -> Result<DataFrame> {
    ...
    let target_cols: Vec<String> =
        if let Some(serde_json::Value::Array(arr)) = params.get("columns") {
            arr.iter()
