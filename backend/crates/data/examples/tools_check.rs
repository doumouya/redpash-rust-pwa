...
fn main() {
    ...
    let cases: Vec<(&str, &str, Value, String)> = vec![
        ...
        (
            "Fill nulls (ffwd)",
            "fill_nulls",
            json!({ "column": c(1), "strategy": "forward" }),
            c(1),
        ),
        (
            "Change type→float",
            "cast",
            json!({ "column": num_col, "dtype": "float" }),
            num_col.clone(),
        ),
        (
            "Change type→int",
            "cast",
            json!({ "column": age_col, "dtype": "int" }),
            age_col.clone(),
        ),
        (
            "Change type→bool",
            "cast",
            json!({ "column": bool_col, "dtype": "bool" }),
            bool_col.clone(),
        ),
        (
            "Change type→date",
            "cast",
            json!({ "column": date_col, "dtype": "date" }),
            date_col.clone(),
        ),
        (
            "Rename column",
            "rename_column",
            json!({ "from": c(0), "to": "client_ref" }),
            c(0),
        ),
        (
            "Drop columns",
        ...
        (
            "Replace text",
            "replace_text",
            json!({ "column": c(2), "find": "@example.fr", "replace": "@redpash.io" }),
            c(2),
        ),
        (
            "Fix invalid",
            "fix_invalid",
            json!({ "sentinels": ["inconnu", "NA", "TBD", "#NAME?", "#VALUE!", "null", "-"] }),
            c(1),
        ),
        (
            "Concatenate cols",
            "join_columns",
            json!({ "col1": c(0), "col2": c(4), "sep": " · ", "new_name": "id_ville" }),
            c(0),
        ),
        (
            "Split column",
            "split_column",
            json!({ "column": c(3), "sep": " ", "keep_original": true }),
            c(3),
        ),
        (
            "Format dates",
            "format_dates",
            json!({ "column": date_col, "fmt": "%Y-%m-%d", "on_incomplete": "keep" }),
            date_col.clone(),
        ),
