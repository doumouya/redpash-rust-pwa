...
> **RETIRED 2026-06-20.** `lean` graduated — most of Phases 1–6 shipped and are now
> tracked as *live* in [`../internal/capability-ledger.md`](../internal/capability-ledger.md);
> the remaining work (the connector/codec lineage, the Phase-7 ops layer) lives there as
> `[gap]`s. The live doc⇄code maps are [`../REDMAP.md`](../REDMAP.md) + [`../INDEX.md`](../INDEX.md);
> the locked architecture is [`../decisions/day-one.md`](../decisions/day-one.md). This file
> is **history only** — kept for the phase narrative, not maintained.
