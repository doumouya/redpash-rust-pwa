...
- **Lean slim (CAS_C8A9):** the case/comment helpers (`list_cases`/`count_cases`/
  ...
  **removed** with the trimmed cases route — only `list_categories` survives. The
  ...
  `users_share_company`, `user_sole_owner_objects`, `create_company`,
  `delete_company`, `delete_membership`, `insert_membership` — admin/monitoring SQL
  joins those). The full multi-tenant set is in the `full-app-pre-slim` snapshot.
...
- **`create_company(avatar_url: Option<&str>)`** — the helper takes an `avatar_url` param so a create flow can seat it without a follow-up PATCH. The INSERT binds `(redpash_id, name, slug, avatar_url)`. The bootstrap call site passes `None` (the canonical internal company doesn't ship an avatar). Mirrors `insert_user(avatar_url: Option<&str>)` in `db/users.rs`.
