...
- **Memberships create modal** (createSpec) supports 3 scopes today — `project`, `company`, `case` (team scope deferred to the teams-CRUD slice; no team entities exist yet). Two select fields are per-scope:
  - `role` — RBAC-tier grant (`owner` / `admin` / `member` / `viewer`); allow-list per scope.
  ...
  - Both lists mirror `routes/admin.rs` allow-lists exactly — drift = 400 from the validator. The entity-picker for `scope_id` uses `labelKeyFn(state)` (cases → `title`, projects/companies → `name`) since the rest field names differ.
