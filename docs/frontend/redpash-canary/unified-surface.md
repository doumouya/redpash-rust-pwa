...
**Coverage** — what this one atom set already composes:
...
The coverage is the proof: the same atoms composing 100% of two
separate pages means the unified surface is **already true at the
component layer** — those pages differ only in page chrome, not in
parts. The remaining **20%** on Cleaner + canvas is the one place the
atom set does not yet reach; that gap *is* the next atom to identify
and name.
