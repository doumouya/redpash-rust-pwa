...
| [`scripts/main.js`](../../../../frontend/scripts/main.js) (shell) | Route entry `{ path: "/objects", chrome: "full", … }`. The `/cleaner` / `/reports` / `/dashboards` routes were retired with the Workspace consolidation — only `/workspace` is mounted now; the per-type pages are modes on the unified Workspace surface. |
